...
def test_current_window():
    ...
    print(
        f"Total: {len(with_auto_id)} / {len(elements)} ({100 * len(with_auto_id) / len(elements):.1f}%)"
    )

    for elem in with_auto_id[:15]:
        print(
            f"  - {elem.get('control_type')}: {elem.get('name')} (id: {elem.get('automation_id')})"
        )
    ...
    if "Notepad" in window_title:
        ...
        for edit in edit_controls:
            print(
                f"    - {edit.get('name')} (automation_id: {edit.get('automation_id')})"
            )
