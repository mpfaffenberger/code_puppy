...
def analyze_element_quality(elements: list[dict]) -> dict:
    ...
    if stats["title_lengths"]:
        stats["avg_title_length"] = sum(stats["title_lengths"]) / len(
            stats["title_lengths"]
        )
...
def compare_with_ocr():
    ...
    try:
        ...
        if not elements:
            print("❌ No accessibility elements found (list is None or empty)")
        ...
        if not ocr_result.text_elements:
            print("❌ No OCR text elements found")
            return
...
def test_compaction_impact():
    ...
    print(
        f"   With good titles: {full_stats['good_title']} → {compact_stats['good_title']}"
    )
    print(
        f"   Interactive: {full_stats['interactive_elements']} → {compact_stats['interactive_elements']}"
    )
    print(
        f"   Static: {full_stats['static_elements']} → {compact_stats['static_elements']}"
    )
            ...
            if cmd == "quit" or cmd == "q":
                ...
                if role_filter:
                    filtered = [
                        e
                        for e in elements
                        if (e.get("role") or e.get("type") or "").lower()
                        == role_filter.lower()
                    ]
                ...
                print(
                    "❌ Unknown command. Try: list, show <index>, stats, compare, refresh, quit"
                )
...
def main():
    ...
    parser.add_argument(
        "--full-tree", action="store_true", help="Show full element tree"
    )
    ...
    parser.add_argument(
        "--test-compaction", action="store_true", help="Test compaction impact"
    )

    args = parser.parse_args()

    print_header("GUI-Cub Accessibility Diagnostic Tool")
