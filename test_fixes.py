...
try:
    ...
    mock_match = TextBoundingBox(
        text="PowerShell",
        confidence=1.0,
        x=84, y=45, width=100, height=20,
        center_x=84, center_y=45
    )
    ...
    print(f"  Before compaction: found={full_result.found}, matches={len(full_result.matches)}, best_match={full_result.best_match is not None}")
    
    # Compact the result
    compact_result = _compact_ocr_find_result(full_result)
    
    print(f"  After compaction: found={compact_result.found}, matches={len(compact_result.matches)}, best_match={compact_result.best_match is not None}")
