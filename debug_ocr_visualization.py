...
from code_puppy.tools.gui_cub.ocr.extraction import prepare_ocr_image, extract_text_from_image
from code_puppy.tools.gui_cub.platform import get_screen_scale_factor
from code_puppy.tools.gui_cub.window_control import _get_active_window_bounds_impl, _focus_window_impl
...
def main():
    ...
    if scale_factor > 1.0:
        print(f"Retina display detected - images will be normalized {scale_factor}x → 1x for OCR\n")
    else:
    ...
    if not args.fullscreen:
        ...
        if bounds.success and bounds.x is not None:
            ...
            print(f"✅ Window bounds (logical): ({bounds.x}, {bounds.y}, {bounds.width}, {bounds.height})")
    ...
    normalized_1x = prepare_ocr_image(screenshot_2x, 1.0)  # Force scale_factor=1.0 (no downscaling)
    print(f"✅ Normalized: {normalized_1x.width}x{normalized_1x.height} pixels ({normalized_1x.mode})\n")
