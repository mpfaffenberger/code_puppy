...
class TestTaskPersistence:
    ...
    def temp_schedules_file(self, tmp_path):
        ...
        with patch("code_puppy.scheduler.config.SCHEDULES_FILE", str(schedules_file)):
            with patch("code_puppy.scheduler.config.SCHEDULER_LOG_DIR", str(schedules_dir)):
                yield schedules_file

    def test_load_tasks_empty_file(self, temp_schedules_file):
        """Test loading tasks when file doesn't exist."""
        with patch("code_puppy.scheduler.config.SCHEDULES_FILE", str(temp_schedules_file)):
            tasks = load_tasks()
            assert tasks == []
    ...
    def test_save_and_load_tasks(self, temp_schedules_file):
        ...
        with patch("code_puppy.scheduler.config.SCHEDULES_FILE", str(temp_schedules_file)):
            with patch("code_puppy.scheduler.config.SCHEDULER_LOG_DIR", str(temp_schedules_file.parent / "logs")):
                tasks = [
    ...
    def test_toggle_task(self, temp_schedules_file):
        ...
        with patch("code_puppy.scheduler.config.SCHEDULES_FILE", str(temp_schedules_file)):
            with patch("code_puppy.scheduler.config.SCHEDULER_LOG_DIR", str(temp_schedules_file.parent / "logs")):
                task = ScheduledTask(id="tog123", name="Toggle Me", prompt="Toggle", enabled=True)
                add_task(task)
