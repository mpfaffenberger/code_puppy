...
def test_sync_mcp_adapter(monkeypatch, tmp_path):
    ...
    mcp_data = {"mcpServers": {"test_server": {"command": "node", "args": ["test.js"]}}}
    ...
    initial_user_servers = {
        "mcp_servers": {"user_server": {"command": "python", "args": ["-m", "test"]}}
    }
    ...
    assert (
        servers2["test_server_2"]["_managed_by"] == "claude_plugin_adapter:test-plugin"
    )
