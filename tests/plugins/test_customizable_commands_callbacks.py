...
class TestGlobalCommands:
    ...
    def test_global_directory_in_command_directories(self):
        ...
        # The conftest ``isolate_global_state_between_tests`` fixture repoints
        # ``_COMMAND_DIRECTORIES[0]`` at the temp CONFIG_DIR (so tests never read
        # the developer's real ~/.code_puppy/commands/). That means the plugin's
        # global-commands entry tracks ``code_puppy.config.CONFIG_DIR`` during
        # tests, so we compare against it directly.
