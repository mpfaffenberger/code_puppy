...
def test_mcp_context7_end_to_end(cli_harness: CliHarness) -> None:
    ...
    try:
        ...
        try:
            result.child.expect(
                re.compile(r"Enter custom name .*\[context7\]"), timeout=15
            )
            result.sendline("\r")
        ...
        try:
            result.child.expect(
                re.compile(
                    r"Required Environment Variables|Proceed with installation\?"
                ),
                timeout=30,
            )
            time.sleep(0.2)
        ...
        result.child.expect(
            re.compile(r"context7.*(running|healthy|ready)"), timeout=60
        )
        cli_harness.wait_for_ready(result)
        ...
        # Prompt intended to trigger an actual tool call
        result.sendline("Use context7 to search for latest pydantic AI information\r")
        time.sleep(10)
        log = result.read_log().lower()
        # Evidence that context7 was actually invoked
        assert "context7" in log and (
            ("tool" in log and "call" in log)  # common
            or ("execute" in log)  # also common
            or ("pydantic" in log)  # fall-back
        )
