...
from tests.conftest import integration_env, log_dump, retry_policy, cli_harness, spawned_cli
from tests.integration.cli_expect.harness import CliHarness, SpawnResult


def test_harness_bootstrap_write_config(cli_harness: CliHarness, integration_env: dict[str, str]) -> None:
...
def test_log_dump_path_exists(tmp_path: pathlib.Path) -> None:
    """Log dump fixture should yield a path under the shared tmp_path."""
    path = log_dump(tmp_path)
    assert path.parent == tmp_path
    assert not path.exists()  # not written until after test


def test_spawned_cli_is_alive(spawned_cli) -> None:
    """spawned_cli fixture should hand us a live CLI at the task prompt."""
    assert spawned_cli.child.isalive()
    # If we look at the log it should contain "Enter your coding task"
    log = spawned_cli.read_log()
    assert "Enter your coding task" in log or log == ""  # streaming may have emptied it


def test_send_command_returns_output(spawned_cli) -> None:
    """send_command should send text and give us back whatever was written."""
    # Use meta command that doesn't reach the LLM
    spawned_cli.sendline("/set owner_name 'HarnessTest'")
    # Allow a moment for the response
    import time
    time.sleep(0.5)
    log = spawned_cli.read_log()
    # We at least expect our command echoed
    assert "/set owner_name" in log or log == ""


def test_harness_cleanup_terminates_and_removes_temp_home(cli_harness: CliHarness, integration_env: dict[str, str]) -> None:
