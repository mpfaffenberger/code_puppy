...
@pytest.mark.skipif(not has_required_api_keys(), reason="Need at least one API key for round-robin testing")
...
"""
    ...
    try:
        ...
        assert "round_robin" in full_log or "glm-4.6" in full_log or "qwen" in full_log.lower()
        
        # Count number of responses to ensure we got responses for all prompts
        response_count = full_log.count("response") or full_log.count("answer") or len([line for line in full_log.split("\n") if any(char.isdigit() for char in line)])
        assert response_count >= len(prompts) // 2  # At least half the prompts should have responses
...
def test_round_robin_rotate_every_parameter(
    ...
    extra_models_file = pathlib.Path(os.environ.get("HOME", tmp_path)) / ".code_puppy" / "extra_models.json"
...
"""
    ...
    try:
        ...
        for i in range(6):
            cli_harness.send_command(result, f"Prompt {i+1}: just say 'response {i+1}'")
            cli_harness.wait_for_ready(result)
