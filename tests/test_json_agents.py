...
class TestJSONAgentDiscovery:
    ...
    def test_discover_json_agents(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            monkeypatch.setattr('code_puppy.config.get_user_agents_directory', lambda: temp_dir)
            ...
            agent1_path = Path(temp_dir) / "agent1.json"  # Changed from agent1-agent.json
            ...
            invalid_path = Path(temp_dir) / "invalid.json"  # Changed from invalid-agent.json
    ...
    def test_discover_nonexistent_directory(self, monkeypatch):
        ...
        monkeypatch.setattr('code_puppy.config.get_user_agents_directory', lambda: '/nonexistent/directory')
        agents = discover_json_agents()
        assert agents == {}
