...
class TestRateLimitIntegration:
    ...
    def test_rate_limit_increase_demonstration(self):
        ...
        print(f"Default pydantic-ai rate limit: {default_limits.request_limit} requests")
        print(f"Code-puppy custom rate limit: {custom_limits.request_limit} requests")
        print(f"Increase: {custom_limits.request_limit - default_limits.request_limit} requests ({((custom_limits.request_limit / default_limits.request_limit) - 1) * 100:.0f}% increase)")
        
        # Verify the change
        assert default_limits.request_limit == 50, "Default should be 50"
        assert custom_limits.request_limit == 100, "Custom should be 100"
        assert custom_limits.request_limit == default_limits.request_limit * 2, "Should be doubled"
    ...
    def test_usage_limits_applied_consistently(self):
        ...
        assert agent_result.request_limit == main_result.request_limit == tui_result.request_limit == 100
    ...
    def test_rate_limit_change_summary(self):
        ...
        summary = {
            ...
            'increase_amount': custom_limits.request_limit - default_limits.request_limit,
            'increase_percentage': ((custom_limits.request_limit / default_limits.request_limit) - 1) * 100,
        ...
        print(f"Increase: +{summary['increase_amount']} requests/minute ({summary['increase_percentage']:.0f}% increase)")
