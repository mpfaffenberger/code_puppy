...
@pytest.mark.asyncio
async def test_other_text_flushed_without_enter(monkeypatch):
    """Regression: typing in 'Other' then submitting WITHOUT pressing Enter in
    the input must still capture the custom answer (previously dropped)."""
    from textual.widgets import Input
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause(0.1)
        other_input = modal.query_one("#other-input", Input)
        other_input.value = "SQLite"  # user types but does NOT press Enter
        modal.action_submit()  # straight to Ctrl+S / Submit
        await pilot.pause(0.1)
    answer = captured[0].answers[0]
    assert answer["selected_options"] == ["Other"]
    assert answer["other_text"] == "SQLite"
