...
class TestDesktopVisualAnalysisResult:
    ...
    def test_confidence_validation_high(self):
        result = DesktopVisualAnalysisResult(
            answer="Test", confidence=1.0, observations="Max confidence"
        )
        assert result.confidence == 1.0

    def test_confidence_validation_low(self):
        result = DesktopVisualAnalysisResult(
            answer="Test", confidence=0.0, observations="Min confidence"
        )
    ...
    def test_confidence_must_be_in_range(self):
        ...
        with pytest.raises(Exception):  # Pydantic validation error
            DesktopVisualAnalysisResult(
                answer="Test", confidence=1.5, observations="Invalid"
            )
    ...
    def test_confidence_negative_fails(self):
        ...
        with pytest.raises(Exception):  # Pydantic validation error
            DesktopVisualAnalysisResult(
                answer="Test", confidence=-0.1, observations="Invalid"
            )
...
class TestGetDesktopVQAAgent:
    ...
    @patch("code_puppy.tools.gui_cub.vqa_desktop._get_model_for_vqa")
...
class TestRunDesktopVQAAnalysis:
    ...
    def test_run_analysis_success(self, mock_get_model, mock_emit, mock_get_agent):
        ...
        result = run_desktop_vqa_analysis(
            question="Find the Submit button", image_bytes=image_data
        )
    ...
    def test_run_analysis_failure(
        self, mock_get_model, mock_emit_warn, mock_emit_info, mock_get_agent
    ):
        mock_get_model.return_value = "test-model"
        mock_agent = MagicMock()
    ...
    @patch("code_puppy.messaging.emit_info")  # Fixed
    @patch("code_puppy.tools.gui_cub.vqa_desktop._get_model_for_vqa")
    def test_run_analysis_with_custom_media_type(
        self, mock_get_model, mock_emit, mock_get_agent
    ):
        mock_get_model.return_value = "test-model"
        mock_agent = MagicMock()
        mock_result = MagicMock()
        mock_result.output = DesktopVisualAnalysisResult(
            answer="Found", confidence=0.8, observations="Test"
        )
        mock_agent.run_sync.return_value = mock_result
        mock_get_agent.return_value = mock_agent

        result = run_desktop_vqa_analysis(
            question="Test question", image_bytes=b"fake_jpeg", media_type="image/jpeg"
        )
    ...
    @patch("code_puppy.messaging.emit_info")  # Fixed
    @patch("code_puppy.tools.gui_cub.vqa_desktop._get_model_for_vqa")
    def test_run_analysis_emits_image_size(
        self, mock_get_model, mock_emit, mock_get_agent
    ):
        mock_get_model.return_value = "test-model"
        mock_agent = MagicMock()
        mock_result = MagicMock()
        mock_result.output = DesktopVisualAnalysisResult(
            answer="Test", confidence=0.5, observations="Test"
        )
        mock_agent.run_sync.return_value = mock_result
        mock_get_agent.return_value = mock_agent
        ...
        run_desktop_vqa_analysis(question="Test", image_bytes=image_data)
    ...
    @patch("code_puppy.messaging.emit_info")  # Fixed
    @patch("code_puppy.tools.gui_cub.vqa_desktop._get_model_for_vqa")
    def test_run_analysis_truncates_long_question(
        self, mock_get_model, mock_emit, mock_get_agent
    ):
        mock_get_model.return_value = "test-model"
        mock_agent = MagicMock()
        ...
        # Very long question
        long_question = "x" * 200
        run_desktop_vqa_analysis(question=long_question, image_bytes=b"test")
    ...
    @patch("code_puppy.messaging.emit_info")  # Fixed
    @patch("code_puppy.tools.gui_cub.vqa_desktop._get_model_for_vqa")
    def test_run_analysis_truncates_long_answer(
        self, mock_get_model, mock_emit, mock_get_agent
    ):
        mock_get_model.return_value = "test-model"
        mock_agent = MagicMock()
        ...
        result = run_desktop_vqa_analysis(question="Test", image_bytes=b"test")
