...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32", reason="Windows-specific un-minimize verification tests"
)
...
class TestWindowsUnMinimizeVerification:
    ...
    def mock_win32(self):
        ...
        with (
            patch(
                "code_puppy.tools.gui_cub.windows_automation.tools.win32gui"
            ) as mock_gui,
            patch(
                "code_puppy.tools.gui_cub.windows_automation.tools.win32con"
            ) as mock_con,
            patch(
                "code_puppy.tools.gui_cub.windows_automation.tools.WINDOWS_AUTOMATION_AVAILABLE",
                True,
            ),
        ):
            mock_con.SW_RESTORE = 9  # Standard Windows constant
            yield {"gui": mock_gui, "con": mock_con}
    ...
    def test_un_minimize_verifies_window_is_restored(self, mock_win32):
        ...
        from code_puppy.tools.gui_cub.windows_automation.tools import (
        ...
        mock_win32[
            "gui"
        ].IsIconic.return_value = True  # Still minimized after restore attempt
    ...
    def test_un_minimize_partial_success_not_foreground(self, mock_win32):
        ...
        mock_win32[
            "gui"
        ].GetForegroundWindow.return_value = 99999  # Different window in foreground
    ...
    def test_un_minimize_window_not_found(self, mock_win32):
        ...
        with patch(
            "code_puppy.tools.gui_cub.windows_automation.tools.list_windows"
        ) as mock_list:
