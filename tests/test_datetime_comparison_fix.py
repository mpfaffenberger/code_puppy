...
class TestDatetimeComparisonFix:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            problematic_data = {
                "history": [
                    ...
                    {
                        "timestamp": datetime.now(timezone.utc).isoformat(),  # current format
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            invalid_data = {
                "history": [
                    ...
                    {
                        "timestamp": "",
                        "description": "Empty timestamp task"
                    },
                    {
    ...
    def test_datetime_comparison_with_various_formats(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            format_variations = [
                ...
                datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),  # Z format
                datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S"),  # No timezone
                datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f"),  # Microseconds, no tz
                datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f+00:00"),  # Full format
            ]
            ...
            for i, ts_format in enumerate(format_variations):
                history_entries.append({
                    "timestamp": ts_format,
                    "description": f"Format variation {i + 1}"
                })
            ...
            try:
                ...
                if "offset-naive and offset-aware" in str(e):
                    pytest.fail(f"Format handling failed with datetime comparison error: {e}")
                else:
                    raise
    ...
    def test_app_history_loading_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mem.log_task("TUI interaction: help command", extras={"output": "Help displayed"})
            mem.log_task("TUI interaction: file listing", extras={"output": "Files listed"})
            mem.log_task("Interactive task: create new file", extras={"awaiting_user_input": False})
            
            # Now simulate what happens in TUI app load_history_list method
            try:
                # This mimics the exact call from tui/app.py line around 463
                recent_history = mem.get_history(within_minutes=24 * 60)  # Last 24 hours
