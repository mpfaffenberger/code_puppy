...
class TestNoVersionUpdateEnvironmentVariable:
    ...
    @pytest.mark.parametrize("env_value", ["1", "true", "TRUE", "yes", "YES", "on", "ON"])
    def test_no_version_update_truthy_values(self, env_value):
        ...
        no_version_update = os.getenv("NO_VERSION_UPDATE", "").lower() in ("1", "true", "yes", "on")
        assert no_version_update is True, f"Failed for environment value: {env_value}"

    @pytest.mark.parametrize("env_value", ["0", "false", "FALSE", "no", "NO", "off", "OFF", "random", ""])
    ...
    def test_main_with_no_version_update_enabled(
        ...
        current_version_displayed = any("Current version: 0.0.90" in call for call in console_calls)
        ...
        assert auto_update_disabled_displayed, "Auto-update disabled message should be displayed"
        
        # Check that latest version is NOT displayed
        latest_version_displayed = any("Latest version:" in call for call in console_calls)
        assert not latest_version_displayed, "Latest version should not be displayed when auto-update is disabled"
    ...
    def test_main_with_no_version_update_disabled(
        ...
        latest_version_displayed = any("Latest version: 0.0.91" in call for call in console_calls)
        ...
        assert update_available_displayed, "Update available message should be displayed"
    ...
    def test_main_with_no_version_update_disabled_same_version(
        ...
        latest_version_displayed = any("Latest version: 0.0.90" in call for call in console_calls)
        assert latest_version_displayed, "Latest version should be displayed"
        
        # Check that NO auto-update messages are displayed (since versions are equal)
        update_available_displayed = any(
            "A new version of code puppy is available" in call 
            for call in console_calls
        )
        assert not update_available_displayed, "Update available message should not be displayed when versions are equal"
    ...
    def test_environment_variable_case_insensitive(self):
        ...
        for env_value, expected in test_cases:
            os.environ["NO_VERSION_UPDATE"] = env_value
            no_version_update = os.getenv("NO_VERSION_UPDATE", "").lower() in ("1", "true", "yes", "on")
            assert no_version_update == expected, f"Failed for environment value: '{env_value}'"
    ...
    def test_console_messages_format(self, mock_console):
        ...
        if no_version_update:
            ...
            mock_console.print("[dim]Auto-update disabled via NO_VERSION_UPDATE environment variable[/dim]")
    ...
    @patch('code_puppy.main.fetch_latest_version')
    def test_fetch_latest_version_not_called_when_disabled(self, mock_fetch_latest_version):
