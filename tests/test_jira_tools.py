...
class TestJiraCreateIssue:
    ...
    def test_create_issue_success(self, mock_context):
        """Test successful issue creation (Task type, no application_service needed)."""
        mock_client = MagicMock()
    ...
    def test_create_issue_without_description(self, mock_context):
        """Test issue creation without description (Task type, no app_service needed)."""
        mock_client = MagicMock()
...
class TestResolveApplicationServiceId:
    ...
    def test_resolve_id_api_error(self):
        ...
        with pytest.raises(ValueError, match="Failed to fetch"):
            _resolve_application_service_id(mock_client, ["Level1", "Level2", "Level3"])
...
class TestJiraCreateIssueWithApplicationService:
    ...
    def test_create_issue_with_app_service_list(self, mock_context):
        ...
        with (
            patch("code_puppy.tools.jira_tools.JiraClient", return_value=mock_client),
            patch(
                "code_puppy.tools.jira_tools.get_application_service_field",
                return_value="customfield_20400",
            ),
...
class TestJiraListApplicationServices:
    ...
    def test_list_with_project_context(self, mock_context):
        ...
        with (
            ...
            result = jira_list_application_services(mock_context, project_key="PROJ")
