...
"""
    ...
    def test_system_prompt_mentions_gui_cub(self, agent):
        ...
        assert "Desktop Automation Cub" in prompt or "gui_cub" in prompt, (
            "System prompt should mention the agent identity"
        )
    ...
    def test_prompt_positions_vqa_as_last_resort(self, agent):
        ...
        assert "VQA" in p
        assert "Last Resort" in p or "Tier 4" in p or "last resort" in p
...
class TestGUICubMinimizedWindowHandling:
    ...
    @pytest.mark.skipif(
        sys.platform != "win32", reason="Windows-specific minimized window test"
    )
    ...
    @pytest.mark.skipif(
        sys.platform != "darwin", reason="macOS-specific minimized window test"
    )
    ...
    @pytest.mark.skipif(
        sys.platform != "win32", reason="Windows-specific tool availability test"
    )
    ...
    @pytest.mark.skipif(
        sys.platform != "darwin", reason="macOS-specific tool availability test"
    )
