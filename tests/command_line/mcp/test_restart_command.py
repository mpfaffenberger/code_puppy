...
class TestRestartCommand:
    ...
    def test_no_loop_refuses_without_touching_server(self, restart_cmd):
        with (
            patch(
                "code_puppy.command_line.mcp.restart_command.find_server_id_by_name",
                return_value="id1",
            ),
            patch("code_puppy.command_line.mcp.restart_command.emit_info") as output,
        ):
            restart_cmd.execute(["a"], group_id="g1")
            assert "running event loop" in str(output.call_args_list)
            restart_cmd.manager.stop_server_sync.assert_not_called()
            restart_cmd.manager.reload_server.assert_not_called()
            restart_cmd.manager.start_server_sync.assert_not_called()
