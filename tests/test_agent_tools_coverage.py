...
class TestRegisterInvokeAgentExecution:
    ...
    @pytest.mark.asyncio
    async def test_invoke_agent_awaits_directly_without_create_task(self):
        """Regression test for contextvars issues caused by create_task()."""
        invoke_agent = self._get_registered_invoke_agent()
        mock_context = MagicMock()
        ...
        mock_result.output = "subagent says hi"
        mock_result.all_messages.return_value = ["msg-1", "msg-2"]

        mock_temp_agent = MagicMock()
        mock_temp_agent.run = AsyncMock(return_value=mock_result)

        terminal_var = MagicMock()
        browser_var = MagicMock()
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("code_puppy.tools.agent_tools.emit_error"))
            stack.enter_context(
                patch(
                    "code_puppy.agents.agent_manager.load_agent",
                    return_value=mock_agent_config,
            ...
            stack.enter_context(
                patch(
                    "code_puppy.model_factory.ModelFactory.load_config",
                    return_value={"test-model": {"provider": "fake"}},
                )
            )
            stack.enter_context(
                patch(
            ...
            stack.enter_context(
                patch("code_puppy.tools.agent_tools.get_use_dbos", return_value=False)
            )
            stack.enter_context(
                patch("code_puppy.tools.agent_tools.get_value", return_value=False)
            )
            stack.enter_context(
                patch(
                    "code_puppy.tools.agent_tools._load_session_history",
            ...
            stack.enter_context(
                patch(
                    "code_puppy.tools.agent_tools.asyncio.create_task",
                    side_effect=AssertionError(
                        "invoke_agent should not spawn a new asyncio task"
                    ),
                )
            )
            ...
            assert result.error is None
            assert result.response == "subagent says hi"
            assert result.session_id == "test-agent-session-abc123"
            mock_temp_agent.run.assert_awaited_once()
            mock_save_history.assert_called_once()
            terminal_var.reset.assert_called_once_with("terminal-token")
            browser_var.reset.assert_called_once_with("browser-token")
