...
def test_codemap_error_prints():
    console = make_fake_console()
    with patch("code_puppy.tools.code_map.make_code_map", side_effect=Exception("fail")):
        result = handle_meta_command("~codemap", console)
        assert result is True
        assert any("Error generating code map" in str(call) for call in (c.args[0] for c in console.print.call_args_list))

def test_m_sets_model():
    console = make_fake_console()
    with patch("code_puppy.command_line.model_picker_completion.update_model_in_input", return_value='some_model'), \
         patch("code_puppy.command_line.model_picker_completion.get_active_model", return_value='gpt-9001'), \
         patch("code_puppy.agent.get_code_generation_agent") as mock_get_agent:
        result = handle_meta_command("~m gpt-9001", console)
        assert result is True
        # Should show confirmation with the model name
        console.print.assert_any_call(
            '[bold green]Active model set and loaded:[/bold green] [cyan]gpt-9001[/cyan]'
        )
        mock_get_agent.assert_called_once_with(force_reload=True)

def test_m_unrecognized_model_lists_options():
    console = make_fake_console()
    with patch("code_puppy.command_line.model_picker_completion.update_model_in_input", return_value=None), \
         patch("code_puppy.command_line.model_picker_completion.load_model_names", return_value=['a', 'b', 'c']):
        result = handle_meta_command("~m not-a-model", console)
        assert result is True
        assert any("Available models" in str(call) for call in (c.args[0] for c in console.print.call_args_list))
        assert any("Usage:" in str(call) for call in (c.args[0] for c in console.print.call_args_list))
...
def test_set_config_value_space():
    ...
    with patch("code_puppy.config.set_config_value") as mock_set_cfg, \
        ...
        assert result is True
        mock_set_cfg.assert_called_once_with('pony', 'rainbow')
        assert any("Set" in str(call) and "pony" in str(call) and "rainbow" in str(call)
                   for call in (c.args[0] for c in console.print.call_args_list))
...
def test_set_config_only_key():
    ...
    with patch("code_puppy.config.set_config_value") as mock_set_cfg, \
        ...
        assert any("Set" in str(call) and "pony" in str(call)
                   for call in (c.args[0] for c in console.print.call_args_list))

def test_show_status():
    console = make_fake_console()
    with patch('code_puppy.command_line.model_picker_completion.get_active_model', return_value='MODEL-X'), \
        ...
        assert result is True
        assert any("Puppy Status" in str(call) and "Ivan" in str(call) and "Biscuit" in str(call) and "MODEL-X" in str(call)
                   for call in (c.args[0] for c in console.print.call_args_list))
...
def test_unknown_meta_command():
    ...
    assert result is True
    assert any("Unknown meta command" in str(call) for call in (c.args[0] for c in console.print.call_args_list))

def test_bare_tilde_shows_current_model():
    console = make_fake_console()
    with patch("code_puppy.command_line.model_picker_completion.get_active_model", return_value="yarn"):
        result = handle_meta_command("~", console)
        assert result is True
        assert any("Current Model:" in str(call) and "yarn" in str(call) for call in (c.args[0] for c in console.print.call_args_list))
...
def test_set_no_args_prints_usage():
    ...
    with patch("code_puppy.config.get_config_keys", return_value=['foo', 'bar']):
        ...
        assert result is True
        assert any("Usage" in str(call) and "Config keys" in str(call)
                   for call in (c.args[0] for c in console.print.call_args_list))
...
def test_set_missing_key_errors():
    ...
    with patch("code_puppy.config.get_config_keys", return_value=['foo', 'bar']):
        ...
        assert result is True
        assert any("You must supply a key" in str(call)
                   for call in (c.args[0] for c in console.print.call_args_list))

def test_cd_lists_dir_exception():
    console = make_fake_console()
    # Simulate error in make_directory_table
    with patch("code_puppy.command_line.utils.make_directory_table", side_effect=Exception("fail-folder")):
        result = handle_meta_command("~cd", console)
        assert result is True
        assert any("Error listing directory" in str(call) for call in (c.args[0] for c in console.print.call_args_list))
...
def test_bare_tilde_with_spaces():
    console = make_fake_console()
    with patch("code_puppy.command_line.model_picker_completion.get_active_model", return_value="zoom"):
        result = handle_meta_command("~    ", console)
        assert result is True
        assert any("Current Model:" in str(call) and "zoom" in str(call) for call in (c.args[0] for c in console.print.call_args_list))
