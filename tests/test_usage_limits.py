...
class TestUsageLimits:
    ...
    def test_usage_limits_default_vs_custom(self):
        ...
        assert default_limits.response_tokens_limit == custom_limits.response_tokens_limit
    ...
    def test_usage_limits_configuration_values(self):
        ...
        assert limits.request_tokens_limit is None, "Request tokens limit should be None (unlimited)"
        assert limits.response_tokens_limit is None, "Response tokens limit should be None (unlimited)"
        assert limits.total_tokens_limit is None, "Total tokens limit should be None (unlimited)"
    ...
    def test_agent_creation_with_mocked_dependencies(self):
        ...
        with patch('code_puppy.config.get_model_name', return_value='test-model'), \
...
class TestUsageLimitsIntegration:
    ...
    def test_all_entry_points_use_custom_limits(self):
        ...
        assert limits1.request_limit == limits2.request_limit == limits3.request_limit == 100
    ...
    def test_usage_limits_in_code_structure(self):
        ...
        assert main_module.get_custom_usage_limits is agent_module.get_custom_usage_limits
        assert tui_module.get_custom_usage_limits is agent_module.get_custom_usage_limits
...
class TestUsageLimitsRealWorld:
    ...
    def test_usage_limits_object_properties(self):
        ...
        assert limits.request_tokens_limit is None or isinstance(limits.request_tokens_limit, int)
        assert limits.response_tokens_limit is None or isinstance(limits.response_tokens_limit, int)
        assert limits.total_tokens_limit is None or isinstance(limits.total_tokens_limit, int)
