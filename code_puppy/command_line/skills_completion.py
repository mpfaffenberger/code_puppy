...
This mirrors the behavior of `MCPCompleter` but is intentionally simpler:
- Only completes subcommands for `/skills ...`
- For `/skills install ...`, completes bundled skill ids from the catalog

Keep it boring, predictable, and fast. Zen puppy approves.
"""
...
def load_bundled_skill_ids() -> List[str]:
    """Load bundled skill ids from the catalog.

    This is imported lazily (catalog can be somewhat expensive to build and we
    don't want to pay that cost unless the user is actually typing
    `/skills install ...`).
    """
    ...
    try:
        ...
        logger.debug(f"Could not load bundled skill ids: {e}")
        return []
...
class SkillsCompleter(Completer):
    ...
    def _get_bundled_skill_ids(self) -> List[str]:
        ...
        if (
            ...
            self._skill_ids_cache = load_bundled_skill_ids()
    ...
    def get_completions(
        ...
        # Find where /skills actually starts (after any leading whitespace)
        ...
        # Extract everything after /skills (and after the space)
        ...
        if len(parts) >= 1:
            ...
            if subcommand == "install":
                # Case 1: Exactly `install` + space (ready for skill id)
                if len(parts) == 1 and text.endswith(" "):
                    for skill_id in sorted(self._get_bundled_skill_ids()):
                        yield Completion(
                            skill_id,
                            start_position=0,
                            display=skill_id,
                            display_meta="Bundled Skill",
                        )
                    return

                # Case 2: `install` + partial skill id
        ...
        # If we only have one part and there's no trailing space, complete subcommands.
        # (But not when there *is* a trailing space  that means they want args.)
        ...
        # All other cases: we don't provide completion (yet).
