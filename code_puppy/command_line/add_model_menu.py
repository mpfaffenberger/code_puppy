...
from code_puppy.models_dev_parser import ModelsDevRegistry, ModelInfo, ProviderInfo
...
class AddModelMenu:
    ...
    def _initialize_registry(self):
        ...
        """
        try:
            self.registry = ModelsDevRegistry()  # Will try API first, then bundled fallback
    ...
    def _render_model_details(self) -> List:
        ...
        if self.view_mode == "providers":
            ...
            if provider.id in UNSUPPORTED_PROVIDERS:
                ...
                lines.append(("", "\n"))
                lines.append(("fg:ansibrightblack", "  Models from this provider cannot be added."))
                lines.append(("", "\n"))
            ...
            if not model.tool_call:
                ...
                lines.append(("", "\n"))
                lines.append(("fg:ansiyellow", "  This model cannot use tools (file ops,"))
                lines.append(("", "\n"))
                lines.append(("fg:ansiyellow", "  shell commands, etc). It will be very"))
                lines.append(("", "\n"))
            ...
            if model.cost_input is not None or model.cost_output is not None:
                if model.cost_input is not None:
                    lines.append(("fg:ansibrightblack", f"    Input: ${model.cost_input:.6f}/token"))
                    lines.append(("", "\n"))
                if model.cost_output is not None:
                    lines.append(("fg:ansibrightblack", f"    Output: ${model.cost_output:.6f}/token"))
                    lines.append(("", "\n"))
                if model.cost_cache_read is not None:
                    lines.append(("fg:ansibrightblack", f"    Cache Read: ${model.cost_cache_read:.6f}/token"))
                    lines.append(("", "\n"))
            else:
            ...
            if model.context_length > 0:
                lines.append(("fg:ansibrightblack", f"    Context: {model.context_length:,} tokens"))
                lines.append(("", "\n"))
            if model.max_output > 0:
                lines.append(("fg:ansibrightblack", f"    Max Output: {model.max_output:,} tokens"))
                lines.append(("", "\n"))
            ...
            if model.input_modalities or model.output_modalities:
                ...
                if model.input_modalities:
                    lines.append(("fg:ansibrightblack", f"    Input: {', '.join(model.input_modalities)}"))
                    lines.append(("", "\n"))
                if model.output_modalities:
                    lines.append(("fg:ansibrightblack", f"    Output: {', '.join(model.output_modalities)}"))
                    lines.append(("", "\n"))
            ...
            if model.knowledge:
                lines.append(("fg:ansibrightblack", f"    Knowledge: {model.knowledge}"))
                lines.append(("", "\n"))
            
            if model.release_date:
                lines.append(("fg:ansibrightblack", f"    Released: {model.release_date}"))
                lines.append(("", "\n"))
            
            lines.append(("fg:ansibrightblack", f"    Open Weights: {model.open_weights}"))
            lines.append(("", "\n"))
        
        return lines
    
    def _add_model_to_extra_config(self, model: ModelInfo, provider: ProviderInfo) -> bool:
    ...
    def _prompt_for_credentials(self, provider: ProviderInfo) -> bool:
        ...
        if not missing_vars:
            emit_info(f"✅ All required credentials for {provider.name} are already set!")
            return True
        ...
        for env_var in missing_vars:
            ...
            try:
                ...
                if not value:
                    emit_warning(f"Skipped {env_var} - you can set it later with /set {env_var}=<value>")
                    continue
    ...
    def run(self) -> bool:
        ...
        preview_frame = Frame(preview_window, width=Dimension(weight=70), title="Details")
        ...
        if self.result == "unsupported" and self.current_provider:
            reason = UNSUPPORTED_PROVIDERS.get(self.current_provider.id, "Not supported")
            emit_error(f"Cannot add model from {self.current_provider.name}: {reason}")
            return False
        
        # Handle pending credential flow after TUI exits
        if self.result == "pending_credentials" and self.pending_model and self.pending_provider:
            ...
            if not self.pending_model.tool_call:
                ...
                try:
                    confirm = input("\n  Are you sure you want to add this model? (y/N): ").strip().lower()
            ...
            if self._prompt_for_credentials(self.pending_provider):
                ...
                if self._add_model_to_extra_config(self.pending_model, self.pending_provider):
                    self.result = "added"
                    return True
            return False
