...
def _show_color_options(color_type: str):
    ...
    color_categories = {
        ...
        "Special Colors": [
            ...
            ("deep_sky_blue1", "🔷"), ("deep_sky_blue2", "🔷"), ("deep_sky_blue3", "🔷"), ("deep_sky_blue4", "🔷"),
            ("turquoise2", "🔷"), ("turquoise4", "🔷"), ("steel_blue1", "🔷"), ("steel_blue3", "🔷"),
            ("chartreuse1", "🟢"), ("chartreuse2", "🟢"), ("chartreuse3", "🟢"), ("chartreuse4", "🟢"),
            ("gold1", "🟡"), ("gold3", "🟡"), ("rosy_brown", "🔴"), ("indian_red", "🔴")
        ]
    }
    
    # Suggested colors for each type
    if color_type == "additions":
        suggestions = [("green", "🟢"), ("bright_green", "🟢"), ("chartreuse1", "🟢"), ("green3", "🟢"), ("sea_green1", "🟢")]
        emit_info("[bold white on green]🎨 Recommended Colors for Additions:[/bold white on green]")
        for color, emoji in suggestions:
            emit_info(f"  [cyan]{color:<16}[/cyan] [white on {color}]■■■■■■■■■■[/white on {color}] {emoji}")
    elif color_type == "deletions":
        suggestions = [("orange1", "🟠"), ("red", "🔴"), ("bright_red", "🔴"), ("indian_red", "🔴"), ("dark_red", "🔴")]
        emit_info("[bold white on orange1]🎨 Recommended Colors for Deletions:[/bold white on orange1]")
        for color, emoji in suggestions:
            emit_info(f"  [cyan]{color:<16}[/cyan] [white on {color}]■■■■■■■■■■[/white on {color}] {emoji}")
    ...
    if command.startswith("/diff"):
        ...
        if len(tokens) == 1:
            ...
            if current_style == "highlighted":
                emit_info(f"[bold]Additions:[/bold]       [{add_fg} on {add_bg}]■■■■■■■■■■[/{add_fg} on {add_bg}] {add_color}")
                emit_info(f"[bold]Deletions:[/bold]       [{del_fg} on {del_bg}]■■■■■■■■■■[/{del_fg} on {del_bg}] {del_color}")
            if current_style == "text":
                emit_info(f"[bold]Additions:[/bold]       [{add_color}]■■■■■■■■■■[/{add_color}] {add_color}")
                emit_info(f"[bold]Deletions:[/bold]       [{del_color}]■■■■■■■■■■[/{del_color}] {del_color}")
            emit_info("\n[yellow]Subcommands:[/yellow]")
            emit_info("  [cyan]/diff style [style][/cyan]                 Set diff style (text/highlighted)")
            emit_info("  [cyan]/diff additions [color][/cyan]             Set addition color (shows options if no color)")
            emit_info("  [cyan]/diff deletions [color][/cyan]             Set deletion color (shows options if no color)")
            emit_info("  [cyan]/diff show[/cyan]                         Show current configuration with example")
        ...
        if subcmd == "style":
            if len(tokens) == 2:
                ...
                emit_info("  [cyan]text[/cyan]         - Plain text diffs with no highlighting")
                emit_info("  [cyan]highlighted[/cyan]   - Intelligent color pairs for maximum contrast")
...
+added line 4"""
            ...
            if current_style == "highlighted":
                ...
                add_fg, add_bg = _get_optimal_color_pair(add_color, "green")
                del_fg, del_bg = _get_optimal_color_pair(del_color, "orange1")
                emit_info(f"Additions: [{add_fg} on {add_bg}]■■■■■■■■■■[/{add_fg} on {add_bg}] {add_color}")
                emit_info(f"Deletions: [{del_fg} on {del_bg}]■■■■■■■■■■[/{del_fg} on {del_bg}] {del_color}")
            else:
