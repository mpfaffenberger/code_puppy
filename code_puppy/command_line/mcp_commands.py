...
class MCPCommandHandler:
    ...
    def cmd_test(self, args: List[str]) -> None:
        ...
        try:
            ...
            try:
                managed_server.get_pydantic_server()
                emit_info(
    ...
    def cmd_add(self, args: List[str]) -> None:
        ...
        try:
            if args:
                ...
                if server_id:
                    emit_info(
                        f"✅ Added server '{name}' (ID: {server_id})",
                        message_group=group_id,
                    )
    ...
    def _run_interactive_install_wizard(self, group_id: str) -> bool:
        ...
        try:
            emit_info(
                "🧙 Interactive MCP Server Installation Wizard", message_group=group_id
            )
            emit_info("", message_group=group_id)
            ...
            success = self._interactive_configure_server(
                selected_server, server_name, group_id
            )
            return success

        except ImportError:
            emit_info(
                "Server catalog not available, falling back to basic wizard",
                message_group=group_id,
            )
        ...
        while True:
            ...
            if popular:
                ...
                for i, server in enumerate(popular):
                    ...
                    emit_info(
                        f"  {i + 1}. {server.display_name} {''.join(indicators)}",
                        message_group=group_id,
                    )
                    emit_info(
                        f"     {server.description[:80]}...", message_group=group_id
                    )
                emit_info("", message_group=group_id)

            # Prompt for selection
            choice = emit_prompt(
                "Enter server number (1-5), 'search <term>' to search, or 'list' to see all categories: "
            )
            ...
            if choice.lower().startswith("search "):
                ...
                if results:
                    ...
                    emit_info(
                        f"No servers found for '{search_term}'", message_group=group_id
                    )
                continue
            ...
            if choice.lower() == "list":
                ...
                for i, category in enumerate(categories):
                    ...
                    emit_info(
                        f"  {i + 1}. {category} ({servers_count} servers)",
                        message_group=group_id,
                    )
                ...
                if cat_choice.isdigit():
                    try:
                        ...
                        if 0 <= index < len(categories):
                            category_servers = catalog.get_by_category(
                                categories[index]
                            )
                            emit_info(
                                f"\n📦 {categories[index]} Servers:",
                                message_group=group_id,
                            )
                            for i, server in enumerate(category_servers):
                                ...
                                emit_info(
                                    f"     {server.description[:80]}...",
                                    message_group=group_id,
                                )

                            server_choice = emit_prompt(
        ...
        while True:
            ...
            if existing_server:
                override = emit_prompt(
                    f"Server '{name}' already exists. Override it? [y/N]: "
                )
    ...
    def _interactive_configure_server(
        self, selected_server, server_name: str, group_id: str
    ) -> bool:
        ...
        if not self._interactive_confirm_installation(
            selected_server, server_name, env_vars, cmd_args, group_id
        ):
            return False

        # Step 5: Install the server
        return self._interactive_install_server(
            selected_server, server_name, env_vars, cmd_args, group_id
        )

    def _interactive_check_system_requirements(
        self, requirements, group_id: str
    ) -> bool:
    ...
    def _interactive_collect_cmd_args(self, requirements, group_id: str) -> dict:
        ...
        for arg_config in required_args:
            ...
            if value:
                ...
                emit_info(
                    f"⚠️  Required argument '{name}' left empty", message_group=group_id
                )

        emit_info("", message_group=group_id)
        return cmd_args

    def _interactive_confirm_installation(
        self,
        selected_server,
        server_name: str,
        env_vars: dict,
        ...
        if env_vars:
            emit_info(
                f"  Environment variables: {len(env_vars)} set", message_group=group_id
            )

        if cmd_args:
            emit_info(
                f"  Command arguments: {len(cmd_args)} configured",
                message_group=group_id,
            )
    ...
    def _interactive_install_server(
        self,
        selected_server,
        server_name: str,
        env_vars: dict,
        ...
        try:
            ...
            if server_id:
                ...
                emit_info(
                    f"✅ Successfully installed '{server_name}' from {selected_server.display_name}!",
                    message_group=group_id,
                )
                emit_info(
                ...
                emit_info("❌ Failed to register server", message_group=group_id)
                return False
    ...
    def cmd_help(self, args: List[str]) -> None:
        ...
        help_lines.append(
            Text("/mcp start-all", style="cyan") + Text("          Start all servers")
        )
        help_lines.append(
...
/mcp add {"name": "test", "type": "stdio", "command": "echo"}"""
        ...
        try:
            ...
            if not selected_server:
                ...
                if not results:
                    ...
                    emit_info(
                        "Try '/mcp add' to browse available servers",
                        message_group=group_id,
                    )
                    return False
                elif len(results) == 1:
                    ...
                    emit_info(
                        "Please use the exact server ID: '/mcp add <server_id>'",
                        message_group=group_id,
                    )
                    return False
            ...
            emit_info(f"❌ Installation failed: {str(e)}", message_group=group_id)
            emit_info(
                f"[dim]Error details: {traceback.format_exc()}[/dim]",
                message_group=group_id,
            )
            return False
    ...
    def _suggest_similar_servers(self, server_name: str, group_id: str = None) -> None:
        ...
        try:
            ...
            if suggestions:
                emit_info(
                    f"Did you mean: {', '.join(suggestions)}", message_group=group_id
                )
            else:
    ...
    def _show_detailed_server_status(
        self, server_id: str, server_name: str, group_id: str = None
    ) -> None:
        """
        ...
        try:
            ...
            state_display = self._format_state_indicator(
                ServerState(state)
            ...
            if recent_events:
                ...
                for event in recent_events[-5:]:  # Show last 5 events
                    ...
                    emit_info(
                        f"  [dim]{time_str}[/dim] [cyan]{event_type}[/cyan] {message}",
                        message_group=group_id,
                    )

        except Exception as e:
            logger.error(
                f"Error showing detailed status for server '{server_name}': {e}"
            )
            emit_info(f"Failed to get detailed status: {e}", message_group=group_id)

    def _handle_interactive_requirements(
        ...
        if requirements.required_tools:
            ...
            for tool_name, tool_info in tool_status.items():
                if tool_info.available:
                    emit_info(
                        f"✅ {tool_name} ({tool_info.version or 'found'})",
                        message_group=group_id,
                    )
                else:
                    emit_info(
                        f"❌ {tool_name} - {tool_info.error}", message_group=group_id
                    )
            ...
            if missing_tools:
                ...
                for tool in missing_tools:
                    ...
                    emit_info(
                        f"Install {tool}: {suggestions[0]}", message_group=group_id
                    )

                proceed = emit_prompt("Continue installation anyway? (y/N): ")
                if proceed.lower() not in ["y", "yes"]:
                    raise Exception(
                        "Installation cancelled due to missing requirements"
                    )
                ...
                if var in os.environ:
                    ...
                    try:
                        ...
                        if value.strip():
                            ...
                            emit_info(
                                f"[green]Set {var}[/green]", message_group=group_id
                            )
                        else:
                            emit_info(
                                f"[yellow]Skipped {var} (empty value)[/yellow]",
                                message_group=group_id,
                            )
                    except Exception as e:
                        emit_info(
                            f"[yellow]Failed to get {var}: {e}[/yellow]",
                            message_group=group_id,
                        )
            ...
            for arg_config in cmd_args:
                ...
                try:
                    ...
                    if value:
                        ...
                        emit_info(
                            f"[green]Set {name}={value}[/green]", message_group=group_id
                        )
                    elif required:
                        emit_info(
                            f"[yellow]Required argument {name} not provided[/yellow]",
                            message_group=group_id,
                        )

                except Exception as e:
                    emit_info(
                        f"[yellow]Failed to get {name}: {e}[/yellow]",
                        message_group=group_id,
                    )
        ...
        if packages:
            ...
            emit_info(
                f"This server requires: {', '.join(packages)}", message_group=group_id
            )
            emit_info(
                "These will be installed automatically when the server starts.",
                message_group=group_id,
            )
