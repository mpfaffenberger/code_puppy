...
class ModelsDevRegistry:
    ...
    def __init__(self, json_path: str | Path | None = None) -> None:
        ...
        self.provider_models: Dict[str, List[str]] = {}  # Maps provider_id to list of model IDs
    ...
    def _fetch_from_api(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            emit_warning(f"models.dev API returned {e.response.status_code}, using bundled fallback")
            return None
    ...
    def _load_data(self) -> None:
            ...
            if data:
                ...
                if bundled_path.exists():
                    try:
                        ...
                        emit_info("📦 Using bundled models database (models.dev unavailable)")
                    except json.JSONDecodeError as e:
                ...
                if isinstance(models_data, dict):
                    for model_id, model_data in models_data.items():
                        try:
                            ...
                            emit_warning(f"Skipping malformed model {provider_id}::{model_id}: {e}")
                            continue
        ...
        emit_info(f"Loaded {len(self.providers)} providers and {len(self.models)} models")
    ...
    def _parse_model(self, provider_id: str, model_id: str, data: Dict[str, Any]) -> ModelInfo:
    ...
    def get_models(self, provider_id: Optional[str] = None) -> List[ModelInfo]:
        ...
        if provider_id:
            ...
            models = [self.models[f"{provider_id}::{model_id}"] for model_id in model_ids 
    ...
    def search_models(self, 
                     ...
                     capability_filters: Optional[Dict[str, Any]] = None) -> List[ModelInfo]:
        """
        ...
        if query:
            ...
            models = [m for m in models if query_lower in m.name.lower() or query_lower in m.model_id.lower()]
        
        # Filter by capabilities
        if capability_filters:
            for capability, required in capability_filters.items():
                if isinstance(required, bool):
                    models = [m for m in models if m.supports_capability(capability) == required]
                else:
                    # Handle other capability filter types if needed
                    models = [m for m in models if getattr(m, capability, None) == required]
    ...
    def filter_by_cost(self, 
                      ...
                      max_output_cost: Optional[float] = None) -> List[ModelInfo]:
        """
    ...
    def filter_by_context(self, 
                         models: List[ModelInfo],
                         min_context_length: int) -> List[ModelInfo]:
        """
...
def convert_to_code_puppy_config(model: ModelInfo, provider: ProviderInfo) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        affordable_models = registry.filter_by_cost(registry.get_models(), max_input_cost=0.001)
