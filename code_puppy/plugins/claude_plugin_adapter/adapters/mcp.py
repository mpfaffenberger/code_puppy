...
def sync_mcp_adapter(plugin_name: str, uninstall: bool = False) -> None:
    ...
    if mcp_servers_file.exists():
        try:
            ...
            logger.error(
                f"claude_plugin_adapter: Failed to load current mcp_servers.json: {e}"
            )
            return
    ...
    if not uninstall and plugin_mcp_file.exists():
        try:
            ...
            p_servers = plugin_mcp_data.get(
                "mcpServers", plugin_mcp_data.get("mcp_servers", {})
            )
            ...
            logger.error(
                f"claude_plugin_adapter: Failed to process {plugin_mcp_file}: {e}"
            )
