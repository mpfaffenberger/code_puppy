...
from code_puppy.plugins.agent_skills.metadata import parse_yaml_frontmatter, FRONTMATTER_PATTERN
        ...
        for agent_file in agents_dir.glob("*.json"):
            try:
                ...
                if data.get("_managed_by") == managed_tag:
                    agent_file.unlink()
                    logger.debug(f"claude_plugin_adapter: Removed managed agent {agent_file}")
            except Exception as e:
                logger.error(f"claude_plugin_adapter: Failed to process agent {agent_file}: {e}")
    ...
    for md_file in plugin_agents_dir.glob("*.md"):
        try:
            ...
            if not frontmatter or not frontmatter.get("name"):
                logger.warning(f"claude_plugin_adapter: Invalid frontmatter in {md_file}")
                continue
            ...
            if not mapped_tools:
                ...
                mapped_tools = ["agent_run_shell_command", "read_file", "list_files", "replace_in_file", "create_file", "grep_search"]
            ...
            logger.debug(f"claude_plugin_adapter: Installed agent {name} from {plugin_name}")
