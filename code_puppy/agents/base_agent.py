...
from code_puppy.agents._builder import (
    ...
    load_puppy_rules as _load_puppy_rules,
    reload_mcp_servers,
)
...
from code_puppy.agents._history import (
    ...
    hash_message,
    prune_interrupted_tool_calls as _prune_interrupted_tool_calls,
)
...
class BaseAgent(ABC):
    ...
    # ---- Walmart-fork compatibility shims ---------------------------------
    # These thin instance methods exist solely so external callers (the
    # walmart_specific plugin, tools/agent_tools.py, and any sub-agent helpers
    # that want to mock at the agent level) can keep treating these as agent
    # methods. The real implementations live in the sibling helper modules.
    def load_puppy_rules(self) -> Optional[str]:
        return _load_puppy_rules()

    def prune_interrupted_tool_calls(self, messages: List[Any]) -> List[Any]:
        return _prune_interrupted_tool_calls(messages)
