...
class TestChatView(unittest.TestCase):
    ...
    def test_add_message_user(self):
        msg = ChatMessage(type=MessageType.USER, content="Hello", timestamp=datetime.now())
        self.chat_view.add_message(msg)
        self.assertIn(msg, self.chat_view.messages)

    def test_add_message_agent(self):
        msg = ChatMessage(type=MessageType.AGENT, content="Hi there!", timestamp=datetime.now())
        self.chat_view.add_message(msg)
        self.assertIn(msg, self.chat_view.messages)

    def test_add_message_system(self):
        msg = ChatMessage(type=MessageType.SYSTEM, content="System message", timestamp=datetime.now())
        self.chat_view.add_message(msg)
        self.assertIn(msg, self.chat_view.messages)

    def test_add_message_error(self):
        msg = ChatMessage(type=MessageType.ERROR, content="Error occurred", timestamp=datetime.now())
        self.chat_view.add_message(msg)
        self.assertIn(msg, self.chat_view.messages)
