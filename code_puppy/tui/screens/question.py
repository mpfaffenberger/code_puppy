...
class QuestionModal(ModalScreen[QuestionResult]):
    ...
    def _commit_other_text(self, value: str) -> None:
        """Apply a free-form 'Other' answer for the current question.

        Shared by :meth:`on_input_submitted` (Enter in the input) and
        :meth:`action_submit` (flush on modal submit) so a typed-but-not-Entered
        custom answer is never silently dropped.
        """
        qi = self._current
        other_idx = self._other_index(qi)
        text = value.strip()
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        self._commit_other_text(event.value)
    ...
    def action_submit(self) -> None:
        # Flush any text still sitting in a visible 'Other' input that the user
        # never confirmed with Enter, so their custom answer isn't dropped.
        other_input = self.query_one("#other-input", Input)
        if other_input.has_class("visible") and other_input.value.strip():
            self._commit_other_text(other_input.value)
