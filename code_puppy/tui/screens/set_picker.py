...
class SetPickerScreen(ModalScreen[bool]):
    ...
    def _edit(self, key: Optional[str]) -> None:
        ...
        if entry is None:
            return
        setting = entry.setting
        # Special-case: spinner_style gets a live catalogue picker so the
        # user can see and select from all available spinners by name.
        if setting.key == "spinner_style":
            self._edit_spinner(setting)
        elif setting.type_hint == "choice" and setting.valid_values:
    ...
    def _edit_spinner(self, setting) -> None:
        """Spinner-style picker: a live-catalogue dropdown instead of free text."""
        from .base import FilterableListScreen, ListChoice
        ...
        try:
            ...
            # Plugin not loaded — fall back to free-text input.
            self._edit_text(setting)
            return

        from code_puppy.command_line.set_menu_values import display_value

        current = display_value(setting) or ""

        choices = []
        for name, info in sorted(catalogue.items()):
            sample = info.frames[0].strip() if info.frames else ""
            desc = info.description or ""
            if sample and desc:
                label = f"{name}  {sample}  —  {desc}"
            elif sample:
                label = f"{name}  {sample}"
            elif desc:
                label = f"{name}  —  {desc}"
                ...
                label = name
            choices.append(ListChoice(id=name, label=label, active=(name == current)))

        def _picked(value) -> None:
            if value is not None:
                self._apply(setting, value)

        self.app.push_screen(FilterableListScreen("Spinner Style", choices), _picked)
