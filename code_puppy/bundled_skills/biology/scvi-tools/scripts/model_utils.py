...
def evaluate_integration(
    adata, batch_key: str, label_key: str, embedding_key: str = "X_scVI"
) -> Dict[str, float]:
    """
...
def get_marker_genes(
    model, adata, groupby: str, n_genes: int = 10
) -> Dict[str, List[str]]:
    """
    ...
    for group in groups:
        ...
        # Filter and sort
        de_sig = de_results[
            (de_results["is_de_fdr_0.05"]) & (de_results["lfc_mean"] > 0.5)
...
def plot_training_history(model, save_path: Optional[str] = None):
    ...
    if "reconstruction_loss_validation" in model.history:
        axes[1].plot(
            model.history["reconstruction_loss_validation"], label="Validation"
        )
...
def save_results(
    ...
    if plot_umap:
        ...
        if rep_key is not None:
            ...
            if batch_cols:
                sc.pl.umap(
                    adata, color=batch_cols[0], ax=axes[0], show=False, title="By Batch"
                )
...
def auto_select_model(adata) -> str:
    ...
    batch_cols = [
        c for c in adata.obs.columns if "batch" in c.lower() or "sample" in c.lower()
    ]
    ...
    if has_batch:
        ...
        suggestions.append(
            {"model": "scVI", "reason": "Standard scRNA-seq analysis", "priority": 2}
        )
    ...
    for key in embedding_keys:
        ...
        try:
            ...
            metrics["integration_score"] = (
                metrics["silhouette_label"] - metrics["silhouette_batch"]
            )
...
def quick_clustering(
    adata, use_rep: str = None, resolution: float = 1.0, n_neighbors: int = 15
):
    """
