...
def train_scanvi(
    adata, batch_key=None, labels_key=None, n_latent=30, n_layers=2, max_epochs=200
):
...
def train_totalvi(
    adata, batch_key=None, protein_key="protein_expression", n_latent=20, max_epochs=200
):
...
Examples:
    ...
    parser.add_argument(
        "--n-latent", type=int, default=30, help="Latent dimensions (default: 30)"
    )
    parser.add_argument(
        "--n-layers", type=int, default=2, help="Encoder/decoder layers (default: 2)"
    )
    parser.add_argument(
        "--max-epochs", type=int, default=200, help="Max training epochs (default: 200)"
    )
    ...
    try:
        ...
        print("Error: scanpy required (install scvi-tools + scanpy)")
        sys.exit(1)
    ...
    if args.input.endswith(".h5mu") or args.model == "multivi":
        try:
            ...
            print(
                "Error: mudata required for .h5mu files. Install with: pip install mudata"
            )
            sys.exit(1)
    else:
