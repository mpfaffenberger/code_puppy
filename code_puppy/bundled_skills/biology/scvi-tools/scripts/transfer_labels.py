...
def transfer_labels(
    reference_model, adata_query, max_epochs=100, confidence_threshold=0.5
):
    """
    ...
    print(
        f"  Confident (>= {confidence_threshold}): {n_confident} ({n_confident / adata_query.n_obs * 100:.1f}%)"
    )
...
Examples:
    ...
    parser.add_argument(
        "--reference-adata",
        help="Reference adata used for training (if not saved with model)",
    )
    parser.add_argument(
        "--max-epochs", type=int, default=100, help="Fine-tuning epochs (default: 100)"
    )
    parser.add_argument(
        "--confidence",
        type=float,
    ...
    if args.reference_adata:
        ...
        try:
            ...
            if "no saved anndata" in str(e).lower():
                print(
                    "Error: Model was saved without adata. Please provide --reference-adata"
                )
                sys.exit(1)
            raise
    ...
    pred_df = adata_annotated.obs[
        ["predicted_cell_type", "prediction_confidence", "confident_prediction"]
    ]
