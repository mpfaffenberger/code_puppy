...
def get_cache_dir() -> Path:
    ...
    cache_dir = os.environ.get(
        "NF_CORE_GENOME_CACHE", os.path.expanduser("~/.nf-core/genomes")
    )
...
def download_genome(
    genome_id: str, components: Optional[List[str]] = None, force: bool = False
) -> bool:
    """
...
Examples:
    ...
    list_parser.add_argument(
        "--installed", action="store_true", help="Show only installed genomes"
    )
    ...
    params_parser = subparsers.add_parser(
        "params", help="Get Nextflow params for genome"
    )
