...
def load_pipeline_config(pipeline: str) -> Dict:
    ...
    if not config_file.exists():
        available = [
            f.stem for f in config_dir.glob("*.yaml") if not f.stem.startswith("_")
        ]
        raise ValueError(
            f"Unknown pipeline '{pipeline}'. Available: {', '.join(available)}"
        )
...
def generate_samplesheet(
    ...
    if input_type not in supported_types:
        return None, ValidationResult(
            valid=False,
            errors=[
                f"Pipeline '{pipeline}' does not support {input_type.upper()} input. "
    ...
    if not rows:
        return None, ValidationResult(
            valid=False, errors=["Could not generate any samplesheet rows from files"]
        )
...
def _process_fastq_files(files, config: Dict, single_end: bool) -> List[Dict]:
    ...
    for sample_key, pair_info in sorted(pairs.items()):
        ...
        row = {
            ...
            "fastq_2": str(Path(pair_info["r2"]).absolute())
            if pair_info.get("r2")
            else "",
        }
...
def _write_samplesheet(rows: List[Dict], config: Dict, output_path: str):
    ...
    active_columns = [
        c for c in column_names if any(c in row and row[c] for row in rows)
    ]
...
def _print_preview(rows: List[Dict], config: Dict):
    ...
    for row in rows[:3]:
        values = [
            str(row.get(col, ""))[:40] for col in active_columns
        ]  # Truncate long paths
        print(",".join(values))
    if len(rows) > 3:
...
Supported pipelines: rnaseq, sarek, atacseq
    ...
    parser.add_argument(
        "input", help="Directory with data files, or CSV path for --validate"
    )
