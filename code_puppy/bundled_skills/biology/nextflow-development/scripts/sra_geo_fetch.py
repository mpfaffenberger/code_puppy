...
def cmd_info(args):
    ...
    if args.output_json:
        info = {
            ...
            "n_runs": len(runs),
            "groups": {
                k: {**v, "runs": None, "gsm_ids": list(v.get("gsm_ids", []))}
...
def cmd_groups(args):
    ...
    for i, (key, info) in enumerate(
        sorted(groups.items(), key=lambda x: -x[1]["count"]), 1
    ):
...
def cmd_list(args):
    ...
    if args.filter:
        ...
        for run in runs:
            if (
                strategy_filter
                and run.get("library_strategy", "").upper() != strategy_filter
            ):
                continue
...
def download_fastq_file(
    url: str, output_path: Path, timeout: int = 600
) -> Tuple[str, bool]:
...
def cmd_download(args):
    ...
    if len(sra_studies) > 1:
        print(
            f"SuperSeries detected with {len(sra_studies)} SRA studies: {', '.join(sorted(sra_studies))}"
        )
    else:
    ...
    if selected_subset:
        ...
        fastq_urls = {
            srr: urls for srr, urls in fastq_urls.items() if srr in filtered_srrs
        }
...
Examples:
  ...
  %(prog)s samplesheet GSE110004 \\
    ...
    info_parser = subparsers.add_parser(
        "info", help="Display study information with sample groups"
    )
    ...
    groups_parser = subparsers.add_parser(
        "groups", help="Show sample groups for interactive selection"
    )
    ...
    list_parser.add_argument(
        "--filter", "-f", help="Filter by strategy:layout (e.g., RNA-Seq:PAIRED)"
    )
    ...
    dl_parser.add_argument(
        "--subset", "-s", help="Filter subset (e.g., RNA-Seq:PAIRED)"
    )
    dl_parser.add_argument(
        "--interactive",
        "-i",
    ...
    dl_parser.add_argument(
        "--parallel", "-p", type=int, default=4, help="Parallel downloads"
    )
    dl_parser.add_argument(
        "--timeout", "-t", type=int, default=600, help="Download timeout (sec)"
    )
    ...
    ss_parser.add_argument(
        "--fastq-dir", "-f", required=True, help="Directory with FASTQ files"
    )
    ss_parser.add_argument(
        "--output", "-o", default="samplesheet.csv", help="Output samplesheet"
    )
    ss_parser.add_argument(
        "--pipeline", "-p", help="Target pipeline (auto-detected if not specified)"
    )
