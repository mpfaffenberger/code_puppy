...
def calculate_pipeline_scores(scan_info: Dict, configs: Dict) -> Dict[str, Dict]:
    ...
    for pipeline_name, config in configs.items():
        ...
        if pipeline_name == "sarek":
            ...
            has_tumor = any(
                ind in " ".join(scan_info["filenames"]) for ind in tumor_indicators
            )
            has_normal = any(
                ind in " ".join(scan_info["filenames"]) for ind in normal_indicators
            )
...
def detect_pipeline(directory: str) -> Tuple[str, Dict]:
    ...
    total_files = (
        scan_info["fastq_count"] + scan_info["bam_count"] + scan_info["cram_count"]
    )
    if total_files == 0:
...
def print_results(
    ...
    print(f"\nDirectory: {directory}")
    print(
        f"Files found: {scan_info['fastq_count']} FASTQ, "
    ...
    sorted_pipelines = sorted(
        scores.keys(), key=lambda k: scores[k]["score"], reverse=True
    )
