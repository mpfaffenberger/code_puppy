...
def check_docker() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            name="Docker", passed=True, message="Docker is available and running"
        )
        ...
        return CheckResult(
            name="Docker", passed=False, message=f"Docker check failed: {str(e)}"
        )
...
def check_nextflow() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            name="Nextflow", passed=False, message=f"Nextflow check failed: {str(e)}"
        )
...
def check_java() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            name="Java", passed=False, message=f"Java check failed: {str(e)}"
        )
...
def check_resources() -> CheckResult:
    ...
    try:
        ...
        if cpu_count < 4:
            warnings.append(
                f"Low CPU count ({cpu_count}). Consider --max_cpus {cpu_count}"
            )
        if 0 < mem_gb < 8:
            warnings.append(
                f"Low memory ({mem_gb:.1f}GB). Use --max_memory '{int(mem_gb)}GB'"
            )
        if 0 < disk_gb < 50:
            warnings.append(
                f"Low disk space ({disk_gb:.1f}GB). Pipelines need ~100GB for human data"
            )
...
def run_all_checks() -> EnvironmentReport:
    ...
    return EnvironmentReport(
        ready=ready, checks=checks, recommendations=recommendations
    )
...
def print_report(report: EnvironmentReport):
    ...
    if report.ready:
        ...
        print(
            "\033[91m✗ Environment is NOT READY. Please address the issues above.\033[0m"
        )
