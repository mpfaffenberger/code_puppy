...
def fetch_sra_run_info(geo_id: str, bioproject: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        for uid in id_list:
            ...
            srr_matches = re.findall(
                r'<Run acc="(SRR\d+)"[^>]*total_spots="(\d+)"[^>]*total_bases="(\d+)"',
                runs_xml,
            )
                ...
                runs.append(
                    {
                        ...
                        "layout": layout_match.group(1).upper()
                        if layout_match
                        else "UNKNOWN",
                        "library_strategy": strategy_match.group(1)
                        if strategy_match
                        else "UNKNOWN",
                        "library_source": source_match.group(1)
                        if source_match
                        else "UNKNOWN",
                        "spots": int(spots),
                        "bases": int(bases),
...
def download_file(
    url: str, output_path: Path, timeout: int = 300, show_progress: bool = True
) -> bool:
    """
...
def fetch_pubmed_metadata(pmid: str, max_retries: int = 3) -> Optional[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.debug(
                f"PubMed fetch error for PMID {pmid} (attempt {attempt + 1}): {e}"
            )
            if attempt < max_retries - 1:
                time.sleep(1 * (attempt + 1))
            continue
...
def fetch_sra_run_info_detailed(
    geo_id: str, bioproject: Optional[str] = None
) -> List[Dict]:
    """
    ...
    try:
        ...
        if not id_list:
            ...
            if not bioproject:
                logger.info(
                    f"No direct SRA link for {geo_id}, searching for BioProject..."
                )
                bioproject = fetch_bioproject_from_geo(geo_id)
...
def format_sample_groups_table(groups: Dict[str, Dict]) -> str:
    ...
    lines.append("")
    lines.append(
        f"{'Sample Group':<20} {'Count':>6} {'Layout':<10} {'GSM Range':<25} {'Est. Size':>12}"
    )
    lines.append("-" * 80)
    ...
    lines.append(
        f"{'TOTAL':<20} {total_runs:>6} {'':<10} {'':<25} {format_file_size(total_size):>12}"
    )
