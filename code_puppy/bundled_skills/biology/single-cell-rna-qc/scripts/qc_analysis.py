...
parser.add_argument(
    "--output-dir",
    type=str,
    help="Output directory (default: <input_basename>_qc_results)",
)
parser.add_argument(
    "--mad-counts",
    type=float,
...
print("\nParameters:")
print(
    f"  MAD thresholds: counts={args.mad_counts}, genes={args.mad_genes}, MT%={args.mad_mt}"
)
...
print(
    f"  Found {adata.var['mt'].sum()} mitochondrial genes (pattern: {args.mt_pattern})"
)
print(
    f"  Found {adata.var['ribo'].sum()} ribosomal genes (pattern: {args.ribo_pattern})"
)
...
plot_qc_distributions(
    adata, before_plot, title="Quality Control Metrics - Before Filtering"
)
...
adata.obs["outlier_counts"] = detect_outliers_mad(
    adata, "total_counts", args.mad_counts
)
adata.obs["outlier_genes"] = detect_outliers_mad(
    adata, "n_genes_by_counts", args.mad_genes
)
...
high_mt_mask = apply_hard_threshold(
    adata, "pct_counts_mt", args.mt_threshold, operator=">"
)
...
adata.obs["pass_qc"] = ~(
    adata.obs["outlier_counts"] | adata.obs["outlier_genes"] | adata.obs["outlier_mt"]
)
...
print(
    f"  Cells passing QC: {adata.obs['pass_qc'].sum()} ({adata.obs['pass_qc'].sum() / adata.n_obs * 100:.2f}%)"
)
...
print(
    f"  Cells after filtering: {adata_filtered.n_obs} (removed {n_cells_original - adata_filtered.n_obs})"
)
...
print(
    f"  Genes after filtering: {adata_filtered.n_vars} (removed {n_genes_original - adata_filtered.n_vars})"
)
...
print("\nAfter filtering:")
print(
    f"  Cells: {adata_filtered.n_obs} ({adata_filtered.n_obs / n_cells_original * 100:.1f}% retained)"
)
print(
    f"  Genes: {adata_filtered.n_vars} ({adata_filtered.n_vars / n_genes_original * 100:.1f}% retained)"
)
