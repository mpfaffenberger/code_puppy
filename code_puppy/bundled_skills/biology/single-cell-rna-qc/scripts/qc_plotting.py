...
def plot_qc_distributions(adata, output_path, title="Quality Control Metrics"):
    ...
    # Row 1: Histograms
    axes[0, 0].hist(
        adata.obs["total_counts"], bins=100, color="steelblue", edgecolor="black"
    )
    axes[0, 0].set_xlabel("Total counts per cell")
    axes[0, 0].set_ylabel("Number of cells")
    axes[0, 0].set_title("Distribution of Total Counts")
    axes[0, 0].axvline(
        adata.obs["total_counts"].median(), color="red", linestyle="--", label="Median"
    )
    axes[0, 0].legend()

    axes[0, 1].hist(
        adata.obs["n_genes_by_counts"], bins=100, color="forestgreen", edgecolor="black"
    )
    ...
    axes[0, 2].hist(
        adata.obs["pct_counts_mt"], bins=100, color="coral", edgecolor="black"
    )
    ...
    axes[0, 2].axvline(
        adata.obs["pct_counts_mt"].median(), color="red", linestyle="--", label="Median"
    )
    axes[0, 2].legend()

    # Row 2: Violin plots
    axes[1, 0].violinplot(
        [adata.obs["total_counts"]], positions=[0], showmeans=True, showmedians=True
    )
    ...
    axes[1, 2].violinplot(
        [adata.obs["pct_counts_mt"]], positions=[0], showmeans=True, showmedians=True
    )
...
def plot_filtering_thresholds(adata, outlier_masks, thresholds, output_path):
    ...
    def plot_with_threshold(ax, metric, outlier_mask, n_mads, hard_threshold=None):
        ...
        ax.hist(
            data[~outlier_mask], bins=100, alpha=0.7, label="Pass QC", color="steelblue"
        )
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Thresholds ({n_mads} MADs)",
        )
    ...
    for idx, (metric, label) in enumerate(metrics):
        if metric in outlier_masks and metric in thresholds:
            ...
            plot_with_threshold(
                axes[idx],
                metric,
                outlier_masks[metric],
                thresholds[metric]["n_mads"],
                hard,
            )
