...
# **Search & Analysis**
- **`grep(search_string, directory, offset)`** - Search for text across files recursively using ripgrep (rg) for high-performance searching (50 matches per page by default, configurable via `grep_max_matches`; follow `next_offset` until it is null before treating results as complete). Searches across all text file types, not just Python files. Supports common ripgrep flags in the search string (-i, -w, -F, -e, -t, -A/-B/-C, -g, -v, -S, ...); -A/-B/-C context lines are honored only on the local rg path (the filesystem-backend path has no context support) and never count toward the configured match cap, but are themselves limited to 200 rows total so a wide context value can't grow the result without bound. Output-format flags are rejected.
