...
from .platform import IS_MACOS, IS_WINDOWS, get_screen_scale_factor
...
async def _try_accessibility_api(
    ...
    try:
        if IS_MACOS:
            ...
            emit_info(
                f"[dim]   ❌ Not found via Accessibility API[/dim]",
                message_group=group_id,
            )
            return None
        elif IS_WINDOWS:
            ...
            try:
                ...
                app_win = d.window(title_re=window_title) if window_title else d.active()
                if app_win.exists():
                    ...
                    try:
                        btn = app_win.child_window(title=target_name, control_type="Button")
                        rect = btn.rectangle()
                        ...
                        emit_info(
                            f"[dim]   ❌ Not found via Windows UIA[/dim]",
                            message_group=group_id,
                        )
                        return None
                else:
                    emit_info(
                        f"[dim]   ❌ Active window not found via Windows UIA[/dim]",
                        message_group=group_id,
                    )
                    return None
            ...
            emit_info(
                f"[dim]   ⏭️  Accessibility path not implemented on this platform[/dim]",
                message_group=group_id,
            )
            return None
...
async def _try_geometry_heuristics(
    ...
    if IS_MACOS:
        ...
        button_offset_map = {
            WindowButton.CLOSE: (MacOSTrafficLightOffsets.CLOSE_X, MacOSTrafficLightOffsets.CLOSE_Y),
            WindowButton.MINIMIZE: (MacOSTrafficLightOffsets.MINIMIZE_X, MacOSTrafficLightOffsets.MINIMIZE_Y),
            WindowButton.MAXIMIZE: (MacOSTrafficLightOffsets.MAXIMIZE_X, MacOSTrafficLightOffsets.MAXIMIZE_Y),
        }
        ...
        button_y = bounds_result.y - (MacOSTrafficLightOffsets.TITLE_BAR_HEIGHT - offset_y)
        ...
        button_x = bounds_result.x + bounds_result.width + offset_x + (WindowsControlOffsets.BUTTON_WIDTH // 2)
    ...
    emit_warning(
        f"[yellow]⚠️  Platform not supported for heuristics[/yellow]",
        message_group=group_id,
    )
    return None
    ...
    """Load a button template from disk with caching.
    
    Args:
        button: Which button template to load
        scale_factor: Screen scale factor (1.0, 2.0, or 3.0)
        state: Button state ("focused", "unfocused", "hover")
    ...
    template_name = f"macos_{button.value}_{state}_light_{scale_suffix}.png"
    ...
    if not CV2_AVAILABLE:
        emit_warning(
            f"[yellow]⚠️  OpenCV-dependent template matching removed[/yellow]",
            message_group=group_id,
        )
        return None
    ...
    try:
        ...
        for state in ["focused", "focused_light_2x_hover", "unfocused"]:
            # Load template
            template = _load_template(button, scale_factor, state.split('_')[0])
            if template is None:
                continue
            ...
            if mask is not None:
                result = cv2.matchTemplate(img_bgr, template_bgr, cv2.TM_CCORR_NORMED, mask=mask)
            else:
...
async def _try_color_detection(
    ...
    if not DEPS_AVAILABLE:
        emit_warning(
            f"[yellow]⚠️  PIL/numpy not available for color detection[/yellow]",
            message_group=group_id,
        )
        return None
    ...
    try:
        ...
        emit_info(
            f"[dim]   ❌ No matching color found[/dim]",
            message_group=group_id,
        )
        return None
