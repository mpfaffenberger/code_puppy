...
def _vqa_hover_and_click_impl(
    ...
    try:
        ...
        return VQAHoverResult(
            success=False,
            error="pyautogui not available - cannot control mouse"
        )
    ...
    if window_title:
        emit_info(f"[cyan]🎯 Focusing window: {window_title}[/cyan]", message_group=group_id)
        from .window_control import _focus_window_impl
        try:
            _focus_window_impl(window_title)
            time.sleep(0.5)
        except Exception as e:
            emit_warning(f"[yellow]⚠️  Could not focus window: {e}[/yellow]", message_group=group_id)
    ...
    if not bounds_result.success:
        return VQAHoverResult(
            success=False,
            error=f"Could not get window bounds: {bounds_result.error}"
        )
    ...
    if IS_MACOS:
        ...
        emit_info(f"[dim]🍎 Added {title_bar_height}px for macOS title bar[/dim]", message_group=group_id)
    
    # Step 2: Capture window screenshot
    emit_info(f"[cyan]📸 Capturing window screenshot...[/cyan]", message_group=group_id)
    ...
    emit_info(f"[cyan]🤖 Analyzing with VQA...[/cyan]", message_group=group_id)
    ...
    emit_info(f"[cyan]🔍 Verifying element is under cursor...[/cyan]", message_group=group_id)
    ...
    if click_after_verify and is_verified:
        emit_info(f"[cyan]🖱️  Clicking at ({actual_x}, {actual_y})...[/cyan]", message_group=group_id)
        pyautogui.click(actual_x, actual_y)
        emit_info(f"[green]✓ Click executed[/green]", message_group=group_id)
...
def register_vqa_hover_tools(agent):
    ...
    def desktop_find_and_hover(
        ...
        result = _vqa_hover_and_click_impl(
            ...
            group_id=context.deps.message_group_id if hasattr(context.deps, "message_group_id") else None,
        )
