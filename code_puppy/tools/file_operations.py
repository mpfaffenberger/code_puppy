...
def list_files(
    context: RunContext | None, directory: str = ".", recursive: bool = True
) -> List[Dict[str, Any]]:
...
def grep(
    context: RunContext | None, search_string: str, directory: str = "."
) -> List[Dict[str, Any]]:
...
def register_file_operations_tools(agent):
    ...
    def list_files(
        ...
        if not os.path.isdir(directory):
            console.print(
                f"[bold red]Error:[/bold red] '{directory}' is not a directory"
            )
            console.print("[dim]" + "-" * 60 + "[/dim]\n")
    ...
    def read_file(context: RunContext, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": f"Cannot read '{file_path}' as text - it may be a binary file"
            }
